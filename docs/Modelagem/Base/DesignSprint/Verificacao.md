...
A verificação por meio de checklists é uma técnica de inspeção que consiste em avaliar sistematicamente um artefato com base em uma lista de critérios ou propriedades predefinidas [1]. Segundo Barbosa et al. [1], esse tipo de avaliação permite identificar de forma objetiva se os requisitos foram atendidos, sem a necessidade de envolver usuários finais, sendo especialmente útil para detectar defeitos e inconsistências antes de testes com usuários. Este checklist foi elaborado com base no backlog definido no artefato [Decision](Decision.md).
...
[1] BARBOSA, Simone D. J. et al. **Interação Humano-Computador e Experiência do Usuário**. Autopublicação, 2021. Capítulo 12 — Métodos de Avaliação de IHC. ISBN: 978-65-00-19677-1.
